...
public class PositionsPersistenceImpl
	 ...
	 */
	@Override
	public List<Positions> findByIsArchive(long is_Archive) {
		return findByIsArchive(
			is_Archive, QueryUtil.ALL_POS, QueryUtil.ALL_POS, null);
	}
	 ...
	 */
	@Override
	public List<Positions> findByIsArchive(
		long is_Archive, int start, int end) {

		return findByIsArchive(is_Archive, start, end, null);
	}
	...
	public List<Positions> findByIsArchive(
		...
		return findByIsArchive(is_Archive, start, end, orderByComparator, true);
	}
	...
	public List<Positions> findByIsArchive(
		...
		if ((start == QueryUtil.ALL_POS) && (end == QueryUtil.ALL_POS) &&
			...
			if (useFinderCache) {
				...
				finderArgs = new Object[] {is_Archive};
			}
		}
		else if (useFinderCache) {
		...
		if (useFinderCache) {
			...
			if ((list != null) && !list.isEmpty()) {
				for (Positions positions : list) {
					if (is_Archive != positions.getIs_Archive()) {
						list = null;
		...
		if (list == null) {
			...
			sb.append(_FINDER_COLUMN_ISARCHIVE_IS_ARCHIVE_2);
	...
	public Positions findByIsArchive_First(
		...
		Positions positions = fetchByIsArchive_First(
	...
	public Positions fetchByIsArchive_First(
		...
		List<Positions> list = findByIsArchive(
			is_Archive, 0, 1, orderByComparator);
	...
	public Positions findByIsArchive_Last(
		...
		Positions positions = fetchByIsArchive_Last(
			is_Archive, orderByComparator);
	...
	public Positions fetchByIsArchive_Last(
		...
		int count = countByIsArchive(is_Archive);
		...
		List<Positions> list = findByIsArchive(
			is_Archive, count - 1, count, orderByComparator);
	...
	public Positions[] findByIsArchive_PrevAndNext(
		...
		try {
			...
			array[0] = getByIsArchive_PrevAndNext(
				session, positions, is_Archive, orderByComparator, true);

			array[1] = positions;

			array[2] = getByIsArchive_PrevAndNext(
				session, positions, is_Archive, orderByComparator, false);
	...
	protected Positions getByIsArchive_PrevAndNext(
		Session session, Positions positions, long is_Archive,
		OrderByComparator<Positions> orderByComparator, boolean previous) {
	...
	public int countByIsArchive(long is_Archive) {
		...
		Object[] finderArgs = new Object[] {is_Archive};
	...
	private static final String _FINDER_COLUMN_ISARCHIVE_IS_ARCHIVE_2 =
		"positions.is_Archive = ?";
